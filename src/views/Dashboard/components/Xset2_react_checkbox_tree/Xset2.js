...
import 'react-checkbox-tree/lib/';
...
const Xset2 = props => {
  ...
  let loadedSet = loadSet(setId);
  
  const [xset, setXset] = useState(loadedSet);
  ...
  const fetchChildNodes = (id) => {
    ...
    return new Promise(resolve => {
      ...
      setTimeout(() => {
        ...
        resolve({
          children: loadChildren(xset.id, id).map((child)=>(buildNodesTree(child)))
        });
      }, 1);
    });
  }
  ...
  function buildNodesTree(item) {
    ...
    let node = {value:item.node, label:item.text, icon: ItemIcons[item.type]};
    if (item.children && item.children.length){
      node.children = item.children.map(child=>buildNodesTree(child));
    }
    return node;
  }

  const nodes = buildNodesTree(xset).children;
    ...
    // <SetContext.Provider value={{ addCheckedState, checkedState }}>
      ...
      >
        ...
        <CardContent className={classes.content}>
          ...
          <WidgetTree nodes={nodes} fetchChildren={fetchChildNodes}/>
        </CardContent>
        <Divider />
        <CardActions className={classes.actions}>
          <Button
